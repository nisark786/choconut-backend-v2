...
class VerifyPaymentView(APIView):
    ...
    def post(self, request):
        ...
        # 3. Get the existing Payment record created during 'CreatePaymentView'
        payment = get_object_or_404(Payment, gateway_order_id=data["razorpay_order_id"])

        # 4. CREATE THE ORDER (From Cart)
        try:
            # We create the order now because payment is confirmed
            ...
            if address_id:
                # Use an existing address belonging to the user
                # We filter by user to ensure security
            ...
            payment.order = order  # Link the payment to the new order
